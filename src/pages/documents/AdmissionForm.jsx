...
import { getAdmissionFormTemplate, saveAdmissionFormTemplate } from '../../api/admissionFormApi';
...
const MANUAL_FIELDS = [
  { key: 'form_no', label: 'Form No', type: 'text', placeholder: 'e.g. ADM-2024-001' },
  { key: 'academic_year', label: 'Academic Year', type: 'text', placeholder: 'e.g. 2024-25' },
  { key: 'previous_school', label: 'Previous School', type: 'text', placeholder: 'e.g. St. Mary School' },
  { key: 'previous_class', label: 'Previous Class', type: 'text', placeholder: 'e.g. Class 9' },
];
...
function FormDocument({ resolvedBody, school }) {
  return (
    <div className="admission-form-page bg-white p-8 max-w-3xl mx-auto shadow-lg border-4 border-double border-gray-400 font-serif">
      {/* School header */}
...
export default function AdmissionForm() {
  ...
  // Load students when generate tab opened
  useEffect(() => {
  ...
  const handleSaveTemplate = async () => {
    setSaving(true);
    try {
      await saveAdmissionFormTemplate(template);
  ...
  const buildVars = () => {
    ...
    return {
      ...
      father_occupation: p?.father?.occupation || '',
      mother_name: p?.mother?.name || '',
      mother_phone: p?.mother?.phone || '',
      mother_occupation: p?.mother?.occupation || '',
  ...
  return (
    <div className="space-y-4">
      ...
      <style>{`
        @media print {
          body.print-admission-form > * { visibility: hidden; }
          body.print-admission-form .admission-form-page,
          body.print-admission-form .admission-form-page * { visibility: visible; }
          body.print-admission-form .admission-form-page {
          ...
          body.print-admission-form .admission-form-page h1 { font-size: 20px !important; }
          body.print-admission-form .admission-form-page h2 { font-size: 14px !important; }
          body.print-admission-form .admission-form-page div { font-size: 11px !important; }
        }
      ...
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-gray-900">Admission Form</h2>
        <p className="text-sm text-gray-500">Design your admission form template and generate for any student</p>
      </div>
      ...
      <div className="flex border-b border-gray-200">
        {[
          ...
          { key: 'generate', label: '📋 Generate Form' },
        ].map((t) => (
          <button
            key={t.key}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 bg-gray-50">
              <span className="text-sm font-medium text-gray-700">Admission Form Template</span>
              <button
            ...
            {templateLoading ? (
              ...
              <textarea
                ...
                placeholder="Write your admission form template here. Click variable buttons above to insert them."
                spellCheck={false}
              />
            )}
          </div>
          ...
          {!templateLoading && template && (
            <div className="space-y-2">
              ...
              <FormDocument resolvedBody={sampleBody} school={{ name: SAMPLE_VALUES.school_name, address: SAMPLE_VALUES.school_address }} />
            </div>
          )}
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-1 space-y-4">
              ...
              {/* Manual fields */}
              <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
                <p className="text-sm font-semibold text-gray-700">Form Details</p>
                {MANUAL_FIELDS.map((f) => (
                  <div key={f.key}>
            ...
            <div className="lg:col-span-2 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-gray-700">Form Preview</p>
                {selectedStudent && (
                  <button
                    onClick={handlePrint}
              ...
              {!selectedStudent ? (
                <div className="bg-white rounded-xl border-2 border-dashed border-gray-200 flex items-center justify-center h-64 text-gray-400 text-sm">
                  Select a student to preview the form
                </div>
              ) : (
                <FormDocument resolvedBody={resolvedBody} school={school} />
              )}
            </div>
          </div>
        </div>
